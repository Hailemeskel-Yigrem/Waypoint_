...
export function registerAccessRoutes(app: FastifyInstance): void {
  ...
  app.register(async (scoped) => {
    ...
    scoped.post('/access/policies', { preHandler: requireRole('owner', 'admin') }, async (request, reply) => {
    ...
    scoped.delete('/access/policies/:policyId', { preHandler: requireRole('owner', 'admin') }, async (request, reply) => {

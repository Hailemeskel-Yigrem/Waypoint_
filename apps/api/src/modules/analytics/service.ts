...
export class AnalyticsService {
  ...
  async dashboard(organizationId: string, from?: Date, to?: Date): Promise<Result<DashboardSummary>> {
    const summary = await this.repo.getDashboard(this.buildQuery(organizationId, from, to));
    return ok(summary);
  }

  async occupancy(organizationId: string, from?: Date, to?: Date): Promise<Result<OccupancyMetrics>> {

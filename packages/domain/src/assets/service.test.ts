...
import { MemoryAssetRepository } from './memory.repository.js';
...
describe('AssetService', () => {
  ...
  beforeEach(() => {
    service = new AssetService(new MemoryAssetRepository());
  });

  it('creates a assets record', async () => {
    const result = await service.create({ organizationId: orgId, name: 'Alpha Asset' });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.name).toBe('Alpha Asset');

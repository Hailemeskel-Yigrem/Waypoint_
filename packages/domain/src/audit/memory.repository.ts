...
export class MemoryAuditEventRepository implements AuditEventRepository {
  ...
  async create(
    input: CreateAuditEventInput & { status: string; createdAt: string; updatedAt: string },
  ): Promise<AuditEvent> {
    const id = randomUUID();
    const row: AuditEvent = {
      ...
      name: input.name,
      status: input.status as AuditEvent['status'],
  ...
  async findById(organizationId: string, id: string): Promise<AuditEvent | null> {
    return this.rows.get(this.key(organizationId, id)) ?? null;
  }

  async findByName(organizationId: string, name: string): Promise<AuditEvent | null> {
  ...
  async update(
    ...
    if (!current) throw new Error('AuditEvent not found');

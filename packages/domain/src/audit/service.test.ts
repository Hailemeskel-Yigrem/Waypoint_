...
import { MemoryAuditEventRepository } from './memory.repository.js';
...
describe('AuditEventService', () => {
  ...
  beforeEach(() => {
    service = new AuditEventService(new MemoryAuditEventRepository());
  });

  it('creates a audit record', async () => {
    const result = await service.create({ organizationId: orgId, name: 'Alpha AuditEvent' });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.name).toBe('Alpha AuditEvent');

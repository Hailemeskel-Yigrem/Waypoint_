...
import { MemoryBuildingRepository } from './memory.repository.js';
...
describe('BuildingService', () => {
  ...
  beforeEach(() => {
    service = new BuildingService(new MemoryBuildingRepository());
  });

  it('creates a buildings record', async () => {
    const result = await service.create({ organizationId: orgId, name: 'Alpha Building' });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.name).toBe('Alpha Building');

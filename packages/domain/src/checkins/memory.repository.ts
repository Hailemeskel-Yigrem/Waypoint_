...
export class MemoryCheckInRepository implements CheckInRepository {
  private readonly rows = new Map<string, CheckIn>();
  ...
  async create(
    ...
    const row: CheckIn = {
      ...
      name: input.name,
      status: input.status as CheckIn['status'],
  ...
  async findById(organizationId: string, id: string): Promise<CheckIn | null> {
    return this.rows.get(this.key(organizationId, id)) ?? null;
  }

  async findByName(organizationId: string, name: string): Promise<CheckIn | null> {
  ...
  async list(filter: CheckInFilter): Promise<CheckIn[]> {
  ...
  async update(organizationId: string, id: string, input: UpdateCheckInInput): Promise<CheckIn> {
    const current = await this.findById(organizationId, id);
    if (!current) throw new Error('CheckIn not found');

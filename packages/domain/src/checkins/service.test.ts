...
import { MemoryCheckInRepository } from './memory.repository.js';
...
describe('CheckInService', () => {
  ...
  beforeEach(() => {
    service = new CheckInService(new MemoryCheckInRepository());
  });

  it('creates a checkins record', async () => {
    const result = await service.create({ organizationId: orgId, name: 'Alpha CheckIn' });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.name).toBe('Alpha CheckIn');

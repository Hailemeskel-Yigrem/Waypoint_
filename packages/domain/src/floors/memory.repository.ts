...
export class MemoryFloorRepository implements FloorRepository {
  private readonly rows = new Map<string, Floor>();
  ...
  async create(
    ...
    const row: Floor = {
      ...
      name: input.name,
      status: input.status as Floor['status'],
  ...
  async findByName(organizationId: string, name: string): Promise<Floor | null> {
  ...
  async list(filter: FloorFilter): Promise<Floor[]> {
  ...
  async update(organizationId: string, id: string, input: UpdateFloorInput): Promise<Floor> {

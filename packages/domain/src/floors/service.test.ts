...
import { MemoryFloorRepository } from './memory.repository.js';
import { FloorService } from './service.js';
...
describe('FloorService', () => {
  ...
  beforeEach(() => {
    service = new FloorService(new MemoryFloorRepository());
  });

  it('creates a floors record', async () => {
    const result = await service.create({ organizationId: orgId, name: 'Alpha Floor' });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.name).toBe('Alpha Floor');

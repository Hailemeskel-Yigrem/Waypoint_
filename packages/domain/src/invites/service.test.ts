...
import { MemoryInviteRepository } from './memory.repository.js';
...
describe('InviteService', () => {
  ...
  beforeEach(() => {
    service = new InviteService(new MemoryInviteRepository());
  });

  it('creates a invites record', async () => {
    const result = await service.create({ organizationId: orgId, name: 'Alpha Invite' });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.name).toBe('Alpha Invite');

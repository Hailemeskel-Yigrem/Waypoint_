...
export class MemoryPolicyRepository implements PolicyRepository {
  private readonly rows = new Map<string, Policy>();
  ...
  async create(
    ...
    const row: Policy = {
      ...
      name: input.name,
      status: input.status as Policy['status'],
  ...
  async findById(organizationId: string, id: string): Promise<Policy | null> {
    return this.rows.get(this.key(organizationId, id)) ?? null;
  }

  async findByName(organizationId: string, name: string): Promise<Policy | null> {
  ...
  async update(organizationId: string, id: string, input: UpdatePolicyInput): Promise<Policy> {

...
import { MemoryReportRepository } from './memory.repository.js';
...
describe('ReportService', () => {
  ...
  beforeEach(() => {
    service = new ReportService(new MemoryReportRepository());
  });

  it('creates a reports record', async () => {
    const result = await service.create({ organizationId: orgId, name: 'Alpha Report' });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.name).toBe('Alpha Report');

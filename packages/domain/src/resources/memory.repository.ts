...
export class MemoryResourceRepository implements ResourceRepository {
  ...
  async create(
    input: CreateResourceInput & { status: string; createdAt: string; updatedAt: string },
  ): Promise<Resource> {
    const id = randomUUID();
    const row: Resource = {
      id,
      ...
      name: input.name,
      status: input.status as Resource['status'],
  ...
  async findByName(organizationId: string, name: string): Promise<Resource | null> {
  ...
  async update(organizationId: string, id: string, input: UpdateResourceInput): Promise<Resource> {

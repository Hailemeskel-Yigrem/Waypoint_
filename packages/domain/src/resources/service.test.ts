...
import { MemoryResourceRepository } from './memory.repository.js';
...
describe('ResourceService', () => {
  ...
  beforeEach(() => {
    service = new ResourceService(new MemoryResourceRepository());
  });

  it('creates a resources record', async () => {
    const result = await service.create({ organizationId: orgId, name: 'Alpha Resource' });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.name).toBe('Alpha Resource');

...
export class MemoryShiftRepository implements ShiftRepository {
  private readonly rows = new Map<string, Shift>();
  ...
  async create(
    ...
    const row: Shift = {
      ...
      name: input.name,
      status: input.status as Shift['status'],
  ...
  async findById(organizationId: string, id: string): Promise<Shift | null> {
  ...
  async update(organizationId: string, id: string, input: UpdateShiftInput): Promise<Shift> {

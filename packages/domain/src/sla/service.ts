...
export class SlaTargetService {
  constructor(private readonly repo: SlaTargetRepository) {}

  // Rule: SLA targets per plan
  // Rule: Breach emits notification

  async create(input: CreateSlaTargetInput): Promise<Result<SlaTarget, Error>> {
  ...
  async get(organizationId: string, id: string): Promise<Result<SlaTarget, Error>> {
    const row = await this.repo.findById(organizationId, id);
    if (!row) return err(new NotFoundError('SlaTarget not found'));
    return ok(row);
  }

  async list(filter: SlaTargetFilter): Promise<Result<SlaTarget[], Error>> {
  ...
  async update(
    ...
    actorRole: string,
  ): Promise<Result<SlaTarget, Error>> {
    ...
    if (!current) return err(new NotFoundError('SlaTarget not found'));

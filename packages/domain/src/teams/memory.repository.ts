...
export class MemoryTeamRepository implements TeamRepository {
  ...
  async create(
    ...
    const row: Team = {
      ...
      name: input.name,
      status: input.status as Team['status'],
  ...
  async findById(organizationId: string, id: string): Promise<Team | null> {
  ...
  async update(organizationId: string, id: string, input: UpdateTeamInput): Promise<Team> {

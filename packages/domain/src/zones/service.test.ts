...
import { MemoryZoneRepository } from './memory.repository.js';
...
describe('ZoneService', () => {
  ...
  beforeEach(() => {
    service = new ZoneService(new MemoryZoneRepository());
  });

  it('creates a zones record', async () => {
    const result = await service.create({ organizationId: orgId, name: 'Alpha Zone' });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.name).toBe('Alpha Zone');

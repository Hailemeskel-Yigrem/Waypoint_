...
export function validateAttachment(input: Record<string, unknown>): ValidationIssue[] {
...
export function assertValidAttachment(input: Record<string, unknown>): void {
  const issues = validateAttachment(input);
  if (issues.length) {

...
export function validateDeskAssignment(input: Record<string, unknown>): ValidationIssue[] {
...
export function assertValidDeskAssignment(input: Record<string, unknown>): void {
  const issues = validateDeskAssignment(input);
  if (issues.length) {

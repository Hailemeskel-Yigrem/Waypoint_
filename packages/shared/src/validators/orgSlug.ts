...
export function validateOrgSlug(input: Record<string, unknown>): ValidationIssue[] {
...
export function assertValidOrgSlug(input: Record<string, unknown>): void {
  const issues = validateOrgSlug(input);
  if (issues.length) {

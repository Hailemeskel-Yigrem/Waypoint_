...
export function validateRecurrence(input: Record<string, unknown>): ValidationIssue[] {
...
export function assertValidRecurrence(input: Record<string, unknown>): void {
  const issues = validateRecurrence(input);
  if (issues.length) {

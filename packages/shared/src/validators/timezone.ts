...
export function validateTimezone(input: Record<string, unknown>): ValidationIssue[] {
...
export function assertValidTimezone(input: Record<string, unknown>): void {
  const issues = validateTimezone(input);
  if (issues.length) {

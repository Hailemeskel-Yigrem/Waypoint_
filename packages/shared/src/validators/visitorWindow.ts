...
export function validateVisitorWindow(input: Record<string, unknown>): ValidationIssue[] {
...
export function assertValidVisitorWindow(input: Record<string, unknown>): void {
  const issues = validateVisitorWindow(input);
  if (issues.length) {

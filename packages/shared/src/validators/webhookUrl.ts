...
export function validateWebhookUrl(input: Record<string, unknown>): ValidationIssue[] {
...
export function assertValidWebhookUrl(input: Record<string, unknown>): void {
  const issues = validateWebhookUrl(input);
  if (issues.length) {
